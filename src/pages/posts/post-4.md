...
сокращенно **ng g c**
На главной странице у нас будет крутой постер из Криминального чтива и поисковиком по центру. Создадим этот компонент:
...
Если открыть app.modules.ts, там уже будет импортирован созданный компонент SearchComponent. Туда же нужно импортировать **HttpClientModule** и добавить массив **imports**.

Мы хотим отобразить инпут для поиска фильмов. При вводе данных должен появляться div с 5 подсказками.
Чтобы получить список подсказок, нужно сделать API запрос [search/movie](https://developers.themoviedb.org/3/search/search-movies). Это можно сделать в **search.components.ts** , но лучше вынести метод в отдельный сервис и внедрить его в компонент. 
...
Для обработки ошибок используем методы библиотеки **Rxjs**
...
Сервис готов, теперь его можно заинджектить в **search.component.ts**.
...
## Итог
Ангуляр очень мощный инструмент и моя поверхостная пробежка по базовым моментам не раскрывает и половины возможностей этого фреймворка. В сравнении с реактом, ощущается громоздким и сложным.      